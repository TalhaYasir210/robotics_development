...
public:
    ExplorerNode() : Node("bot2_explorer") {
        ...
        initial_pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>(initial_pose_topic, 10);
        map_pub_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>(map_topic, rclcpp::QoS(10).transient_local());

        get_qr_client_ = this->create_client<swarm_interfaces::srv::GetQR>("/get_qr");
        get_map_client_ = this->create_client<swarm_interfaces::srv::GetMap>("/get_map");
        manage_loc_client_ = this->create_client<nav2_msgs::srv::ManageLifecycleNodes>("/tb3_2/lifecycle_manager_localization/manage_nodes");
        manage_nav_client_ = this->create_client<nav2_msgs::srv::ManageLifecycleNodes>("/tb3_2/lifecycle_manager_navigation/manage_nodes");
...
private:
    ...
    rclcpp::Client<nav2_msgs::srv::ManageLifecycleNodes>::SharedPtr manage_loc_client_;
    rclcpp::Client<nav2_msgs::srv::ManageLifecycleNodes>::SharedPtr manage_nav_client_;
    ...
    void scanCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
        if (state_ == State::LOCALIZING || state_ == State::DONE) return;
    ...
    void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg) {
        if (!camera_info_ || state_ == State::LOCALIZING || state_ == State::DONE) return;
            ...
            if (n > 0) {
                for (zbar::Image::SymbolIterator symbol = zbar_image.symbol_begin(); symbol != zbar_image.symbol_end(); ++symbol) {
                    ...
                    if ((state_ == State::SERVOING || state_ == State::RECOVERY_SWEEP) && payload == target_qr_payload_) {
                        ...
                        latest_tf_cam_to_qr_.setOrigin(tf2::Vector3(tvec.at<double>(0), tvec.at<double>(1), tvec.at<double>(2)));
    ...
    void startLocalization() {
        ...
        if (!get_qr_client_->wait_for_service(std::chrono::seconds(1))) {
            RCLCPP_ERROR(this->get_logger(), "Database get_qr service not available! Aborting localization.");
            return;
        }
        ...
        RCLCPP_INFO(this->get_logger(), "Asking database for coordinates of '%s'...", target_qr_payload_.c_str());

        get_qr_client_->async_send_request(
            request,
            [this](rclcpp::Client<swarm_interfaces::srv::GetQR>::SharedFuture future) {
                ...
                if (response->success) {
                    ...
                    try {
                        ...
                        geometry_msgs::msg::TransformStamped tf_base_to_cam_msg = tf_buffer_->lookupTransform(base_frame, camera_frame_, tf2::TimePointZero);
                        
                        tf2::Transform tf_base_to_cam;
                        tf2::fromMsg(tf_base_to_cam_msg.transform, tf_base_to_cam);
                        
                        // 2. Calculate Base -> QR
                        tf2::Transform tf_base_to_qr = tf_base_to_cam * latest_tf_cam_to_qr_;
                        ...
                        RCLCPP_INFO(this->get_logger(), "Base localized perfectly via Inverse Kinematics.");
                    ...
                    RCLCPP_WARN(this->get_logger(), "Database does not know '%s' yet. Ignoring it and resuming exploration.", target_qr_payload_.c_str());
    ...
    void fetchMap() {
        ...
        if (!get_map_client_->wait_for_service(std::chrono::seconds(1))) {
            RCLCPP_ERROR(this->get_logger(), "Database get_map service not available!");
            return;
        }
        ...
        get_map_client_->async_send_request(
            request,
            [this](rclcpp::Client<swarm_interfaces::srv::GetMap>::SharedFuture future) {
                auto response = future.get();
                if (response->success) {
                    RCLCPP_INFO(this->get_logger(), "Map retrieved from database successfully!");
                    ...
                    RCLCPP_INFO(this->get_logger(), ">>> ACTIVATING AMCL AND NAV2 LIFECYCLE NODES <<<");
                    ...
                    // 2. Wait for AMCL to transition to Active so it can receive the initialpose topic
                    ...
                    RCLCPP_INFO(this->get_logger(), ">>> AMCL TRIGGERED! Robot Map Position: X: %.2f, Y: %.2f <<<", 
                                final_initialpose_.pose.pose.position.x, final_initialpose_.pose.pose.position.y);
                    
                    RCLCPP_INFO(this->get_logger(), "Explorer Node going DORMANT. Handoff to Nav2 complete!");
                    state_ = State::DONE;
                    ...
                    RCLCPP_WARN(this->get_logger(), "Map not found in database! Retrying in 2 seconds...");
        ...
        get_map_client_->async_send_request(
            ...
            [this](rclcpp::Client<swarm_interfaces::srv::GetMap>::SharedFuture future) {
                ...
                if (response->success) {
                    ...
                    map_msg.header.stamp = this->now();
                    map_pub_->publish(map_msg);
                    RCLCPP_INFO(this->get_logger(), "Updated map published to /tb3_2/map");
                    
                    if (response->is_mapping_complete) {
                        RCLCPP_INFO(this->get_logger(), "Database indicates mapping is complete. Stopping map updates.");
